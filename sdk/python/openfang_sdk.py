"""
Ochi Python SDK — helper library for writing Python agents (legacy module name: openfang_sdk).
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the Ochi kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("OCHI_AGENT_ID", os.environ.get("OPENFANG_AGENT_ID", ""))
        message = os.environ.get("OCHI_MESSAGE", os.environ.get("OPENFANG_MESSAGE", ""))
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the Ochi kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in Ochi daemon logs)."""
...
if __name__ == "__main__":
    print("Ochi Python SDK")
    print("====================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from ochi_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
