...
ochi migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.ochi/` with a summary of everything that was imported, skipped, or needs manual attention.
...
| Item | Source (OpenClaw) | Destination (Ochi) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.ochi/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.ochi/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.ochi/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.ochi/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.ochi/config.toml`.
...
```bash
ochi skill install <skill-name-or-path>
```

Ochi automatically detects OpenClaw-format skills and converts them during installation.
...
This creates `~/.ochi/` with a default `config.toml`.
...
**Ochi** (`~/.ochi/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
Copy any `MEMORY.md` files from OpenClaw agents to Ochi agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.ochi/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | Ochi |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.ochi/config.toml` |
...
Tools were renamed between OpenClaw and Ochi for consistency. The migration engine handles this automatically.

| OpenClaw Tool | Ochi Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Name | Ochi Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (Ochi Wire Protocol) peer-to-peer |
...
```bash
ochi migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
ochi skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.ochi/channels_import.toml
# Copy the [channels.*] sections into ~/.ochi/config.toml
```
