...
> **Commit mới nhất:** Tách SUB-PLAN-TASK.md — Di chuyển chi tiết sub-tasks (DEBT-01 A→Q, DEBT-02 A→J) sang file riêng để dễ quản lý và rollback.
...
### DEBT-01: Đổi tên Triệt để (OpenFang → Ochi) — ✅ **Hoàn tất**

> Loại bỏ hoàn toàn mọi tham chiếu đến `OpenFang`/`openfang`/`OPENFANG` trong toàn bộ codebase (1,271+ lần trong `.rs`, 26 lần trong `.toml`, 26 lần trong `.md`, nhiều file `.js`/`.py`). Gồm 17 sub-tasks (`DEBT-01-A` → `DEBT-01-Q`), tất cả đã hoàn tất.
>
> 📋 **Chi tiết phân công sub-tasks:** [SUB-PLAN-TASK.md — Mục 1](SUB-PLAN-TASK.md#1-debt-01-đổi-tên-triệt-để-openfang--ochi)

---

### DEBT-02: Xử lý `unwrap()` và `expect()` An toàn — ✅ **Hoàn tất**

> Thay thế các lệnh gọi `.unwrap()` và `.expect()` không an toàn bằng các cấu trúc xử lý lỗi phù hợp, ngăn ngừa `panic` trong production. Gồm 10 sub-tasks (`DEBT-02-A` → `DEBT-02-J`), tất cả đã hoàn tất.
>
> 📋 **Chi tiết phân công sub-tasks:** [SUB-PLAN-TASK.md — Mục 2](SUB-PLAN-TASK.md#2-debt-02-xử-lý-unwrap-và-expect-an-toàn)

---

> **Lưu ý cho AI Builder:** Khi thêm sub-tasks mới hoặc cập nhật trạng thái, hãy sửa file [SUB-PLAN-TASK.md](SUB-PLAN-TASK.md) thay vì file này. Đây là bắt buộc theo **Mục 6.2**.
