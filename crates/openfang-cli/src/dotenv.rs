//! Minimal `.env` file loader/saver for `${OCHI_HOME:-...}/.env`.
//!
...
/// Return the path to `<openfang home>/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    Some(openfang_kernel::config::openfang_home().join(".env"))
}

/// Load `<openfang home>/.env` and `<openfang home>/secrets.env` into `std::env`.
///
...
/// Return the path to `<openfang home>/secrets.env`.
...
/// Upsert a key in `<openfang home>/.env`.
///
...
/// Remove a key from `<openfang home>/.env`.
///
...
/// List key names (without values) from `<openfang home>/.env`.
#[allow(dead_code)]
