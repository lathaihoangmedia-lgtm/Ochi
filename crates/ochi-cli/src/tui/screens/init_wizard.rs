...
//!
//! Launched by `ochi init` (without `--quick`). Takes over the terminal,
...
struct State {
    ...
    openclaw_scan: Option<ochi_migrate::openclaw::ScanResult>,
    migration_report: Option<ochi_migrate::report::MigrationReport>,
...
pub fn run() -> InitResult {
    ...
    let (migrate_tx, migrate_rx) =
        std::sync::mpsc::channel::<Result<ochi_migrate::report::MigrationReport, String>>();
...
fn handle_migration_key(
    state: &mut State,
    code: KeyCode,
    migrate_tx: &std::sync::mpsc::Sender<Result<ochi_migrate::report::MigrationReport, String>>,
) {
        ...
        MigrationPhase::Offer => match code {
            ...
            KeyCode::Enter => {
                ...
                if yes {
                    ...
                    let target_dir = home.join(".ochi");
                    let tx = migrate_tx.clone();
                    std::thread::spawn(move || {
                        let options = ochi_migrate::MigrateOptions {
                            source: ochi_migrate::MigrateSource::OpenClaw,
                            source_dir,
                            target_dir,
                            dry_run: false,
                        };
                        let result =
                            ochi_migrate::run_migration(&options).map_err(|e| format!("{e}"));
...
fn save_config(state: &mut State) {
    ...
    let _ = std::fs::create_dir_all(ochi_dir.join("agents"));
    let _ = std::fs::create_dir_all(ochi_dir.join("data"));
    crate::restrict_dir_permissions(&ochi_dir);
...
/// Check if the `ochi-desktop` binary exists next to the current exe.
...
fn draw_migration_done(f: &mut Frame, area: Rect, state: &State) {
    ...
    if let Some(ref error) = state.migration_error {
        ...
        use ochi_migrate::report::ItemKind;
