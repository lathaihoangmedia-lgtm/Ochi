...
impl McpBackend {
    ...
    /// Find agent ID by tool name (strip `ochi_agent_` prefix, match by name).
    fn resolve_tool_agent(&self, tool_name: &str) -> Option<String> {
        let agent_name = tool_name.strip_prefix("ochi_agent_")?.replace('_', "-");
        ...
        let agent_name_underscore = tool_name.strip_prefix("ochi_agent_")?;
        for (id, name, _) in &agents {
...
mod tests {
    ...
    fn test_handle_initialize() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "ochi");
    }
