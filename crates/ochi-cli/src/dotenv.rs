...
pub fn env_file_path() -> Option<PathBuf> {
    Some(ochi_kernel::config::ochi_home().join(".env"))
}
...
pub fn secrets_env_path() -> Option<PathBuf> {
    Some(ochi_kernel::config::ochi_home().join("secrets.env"))
}
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content = String::from("# Ochi environment — managed by `ochi config set-key`\n");
