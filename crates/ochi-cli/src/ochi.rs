// Auto-generated wrapper entrypoint for the `ochi` binary.
mod bundled_agents;
mod dotenv;
mod launcher;
mod mcp;
...
fn cmd_init(quick: bool) {
    ...
    let ochi_dir = ochi_home();

    // --- Ensure directories exist ---
    if !ochi_dir.exists() {
        std::fs::create_dir_all(&ochi_dir).unwrap_or_else(|e| {
            ui::error_with_fix(
                &format!("Failed to create {}", ochi_dir.display()),
                &format!("Check permissions on {}", home.display()),
            );
            eprintln!("  {e}");
            std::process::exit(1);
        });
        restrict_dir_permissions(&ochi_dir);
    }

    for sub in ["data", "agents"] {
        let dir = ochi_dir.join(sub);
        if !dir.exists() {
    ...
    bundled_agents::install_bundled_agents(&ochi_dir.join("agents"));

    if quick {
        cmd_init_quick(&ochi_dir);
    } else {
        cmd_init_interactive(&ochi_dir);
    }
}

/// Quick init: no prompts, auto-detect, write config + .env, print next steps.
fn cmd_init_quick(ochi_dir: &std::path::Path) {
    ui::banner();
    ui::blank();

    let (provider, api_key_env, model) = detect_best_provider();

    write_config_if_missing(ochi_dir, provider, model, api_key_env);
...
fn cmd_init_interactive(ochi_dir: &std::path::Path) {
...
/// Launch the ochi-desktop Tauri app, connecting to the running daemon.
fn launch_desktop_app(_ochi_dir: &std::path::Path) {
    ...
    let desktop_bin = {
        ...
        #[cfg(windows)]
        let name = "ochi-desktop.exe";
        #[cfg(not(windows))]
        let name = "ochi-desktop";
    ...
    match desktop_bin {
        ...
        _ => {
            ...
            ui::hint("Install it with: cargo install ochi-desktop");
...
fn cmd_start(config: Option<PathBuf>) {
    ...
    rt.block_on(async {
        ...
        if let Err(e) =
            ochi_api::server::run_daemon(kernel, &listen_addr, Some(&daemon_info_path)).await
        {
...
fn boot_kernel_error(e: &ochi_kernel::error::KernelError) {
    let msg = e.to_string();
...
fn cmd_doctor(json: bool, repair: bool) {
    ...
    if let Some(h) = &home {
        ...
        if ochi_dir.exists() {
            if !json {
                ui::check_ok(&format!("Ochi directory: {}", ochi_dir.display()));
            }
            checks.push(serde_json::json!({"check": "ochi_dir", "status": "ok", "path": ochi_dir.display().to_string()}));
        } else if repair {
            if !json {
            ...
            if answer.is_empty() || answer.starts_with('y') || answer.starts_with('Y') {
                if std::fs::create_dir_all(&ochi_dir).is_ok() {
                    restrict_dir_permissions(&ochi_dir);
                    for sub in ["data", "agents"] {
                        let _ = std::fs::create_dir_all(ochi_dir.join(sub));
                    }
                    if !json {
            ...
            checks.push(serde_json::json!({"check": "ochi_dir", "status": if repaired { "repaired" } else { "fail" }}));
        } else {
            if !json {
                ui::check_fail("Ochi directory not found. Run `ochi init` first.");
            }
            checks.push(serde_json::json!({"check": "ochi_dir", "status": "fail"}));
            all_ok = false;
        }

        // --- Check 2: .env file exists + permissions ---
        let env_path = ochi_dir.join(".env");
        if env_path.exists() {
            #[cfg(unix)]
            {
                use std::os::unix::fs::PermissionsExt;
            ...
            if !json {
                ui::check_warn(
                    ".env file not found (create with: ochi config set-key <provider>)",
                );
            }
...
decay_rate = 0.05
"#;
                let _ = std::fs::create_dir_all(&ochi_dir);
        ...
        let daemon_json_path = ochi_dir.join("daemon.json");
        ...
        let db_path = ochi_dir.join("data").join("ochi.db");
        if db_path.exists() {
        ...
        #[cfg(unix)]
        {
            if let Ok(output) = std::process::Command::new("df")
                .args(["-m", &ochi_dir.display().to_string()])
                .output()
            {
        ...
        let agents_dir = ochi_dir.join("agents");
                ...
                Ok(cfg) => {
                    ...
                    if !cfg.include.is_empty() {
                        ...
                        for inc in &cfg.include {
                            let inc_path = ochi_dir.join(inc);
                            if inc_path.exists() {
                                if !json {
    ...
    // --- Check 13: Skill registry health ---
    {
        ...
        let skills_dir = home
            .as_ref()
            .map(|_| ochi_home().join("skills"))
            .unwrap_or_else(|| std::path::PathBuf::from("skills"));
        let mut skill_reg = ochi_skills::registry::SkillRegistry::new(skills_dir.clone());
        ...
        for skill in &skills {
            if let Some(ref prompt) = skill.manifest.prompt_context {
                let warnings = ochi_skills::verify::SkillVerifier::scan_prompt_content(prompt);
    ...
    if let Some(ref h) = home {
        ...
        let mut ext_registry =
            ochi_extensions::registry::IntegrationRegistry::new(&ochi_dir);
...
fn cmd_completion(shell: clap_complete::Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "ochi", &mut std::io::stdout());
}
...
fn cmd_migrate(args: MigrateArgs) {
    let source = match args.from {
        MigrateSourceArg::Openclaw => ochi_migrate::MigrateSource::OpenClaw,
        MigrateSourceArg::Langchain => ochi_migrate::MigrateSource::LangChain,
        MigrateSourceArg::Autogpt => ochi_migrate::MigrateSource::AutoGpt,
    };
    ...
    let source_dir = args.source_dir.unwrap_or_else(|| {
        ...
        match source {
            ochi_migrate::MigrateSource::OpenClaw => home.join(".openclaw"),
            ochi_migrate::MigrateSource::LangChain => home.join(".langchain"),
            ochi_migrate::MigrateSource::AutoGpt => home.join("Auto-GPT"),
        }
    });
...
fn cmd_skill_install(source: &str) {
    ...
    if source_path.exists() && source_path.is_dir() {
        ...
        if !manifest_path.exists() {
            ...
            if ochi_skills::openclaw_compat::detect_openclaw_skill(&source_path) {
                println!("Detected OpenClaw skill format. Converting...");
                match ochi_skills::openclaw_compat::convert_openclaw_skill(&source_path) {
                    Ok(manifest) => {
                        let dest = skills_dir.join(&manifest.skill.name);
                        // Copy skill directory
                        copy_dir_recursive(&source_path, &dest);
                        if let Err(e) = ochi_skills::openclaw_compat::write_ochi_manifest(
                            &dest, &manifest,
                        ) {
        ...
        let manifest: ochi_skills::SkillManifest =
...
fn cmd_channel_setup(channel: Option<&str>) {
    ...
    match channel.as_str() {
        ...
        "matrix" => {
            ...
            println!("     (e.g., register @ochi-bot:matrix.org)");
            println!("  2. Obtain an access token:");
            println!("     curl -X POST https://matrix.org/_matrix/client/r0/login \\");
            println!("       -d '{{\"type\":\"m.login.password\",\"user\":\"ochi-bot\",\"password\":\"...\"}}'");
...
pub(crate) fn ochi_home() -> PathBuf {
    ochi_kernel::config::ochi_home()
}
...
fn cmd_integration_add(name: &str, key: Option<&str>) {
    let home = ochi_home();
    let mut registry = ochi_extensions::registry::IntegrationRegistry::new(&home);
    ...
    let vault = if vault_path.exists() {
        let mut v = ochi_extensions::vault::CredentialVault::new(vault_path);
        if v.unlock().is_ok() {
            Some(v)
        } else {
            None
    ...
    let mut resolver =
        ochi_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
    ...
    match ochi_extensions::installer::install_integration(
        &mut registry,
        &mut resolver,
        name,
        &provided_keys,
        ...
        Ok(result) => {
            match &result.status {
                ochi_extensions::IntegrationStatus::Ready => {
                    ui::success(&result.message);
                }
                ochi_extensions::IntegrationStatus::Setup => {
...
fn cmd_integration_remove(name: &str) {
    ...
    match ochi_extensions::installer::remove_integration(&mut registry, name) {
        Ok(msg) => {
            ui::success(&msg);
            // Hot-reload daemon
            if let Some(base_url) = find_daemon() {
...
fn cmd_integrations_list(query: Option<&str>) {
    ...
    let dotenv_path = home.join(".env");
    let resolver =
        ochi_extensions::credentials::CredentialResolver::new(None, Some(&dotenv_path));

    let entries = if let Some(q) = query {
        ochi_extensions::installer::search_integrations(&registry, q)
    } else {
        ochi_extensions::installer::list_integrations(&registry, &resolver)
    };
    ...
    let mut by_category: std::collections::BTreeMap<
        String,
        Vec<&ochi_extensions::installer::IntegrationListEntry>,
    ...
    for (category, items) in &by_category {
        ...
        for item in items {
            let status_badge = match &item.status {
                ochi_extensions::IntegrationStatus::Ready => "[Ready]".green().to_string(),
                ochi_extensions::IntegrationStatus::Setup => "[Setup]".yellow().to_string(),
                ochi_extensions::IntegrationStatus::Available => {
                    "[Available]".dimmed().to_string()
                }
                ochi_extensions::IntegrationStatus::Error(msg) => {
                    format!("[Error: {msg}]").red().to_string()
                }
                ochi_extensions::IntegrationStatus::Disabled => {
...
fn cmd_vault_init() {
    let home = ochi_home();
    let vault_path = home.join("vault.enc");
    let mut vault = ochi_extensions::vault::CredentialVault::new(vault_path);
...
fn cmd_scaffold(kind: ScaffoldKind) {
    ...
    let result = match kind {
        ScaffoldKind::Skill => {
            ochi_extensions::installer::scaffold_skill(&cwd.join("my-skill"))
        }
        ScaffoldKind::Integration => {
            ochi_extensions::installer::scaffold_integration(&cwd.join("my-integration"))
        }
    };
    match result {
...
fn cmd_models_list(provider_filter: Option<&str>, json: bool) {
    if let Some(base) = find_daemon() {
        ...
        let catalog = ochi_runtime::model_catalog::ModelCatalog::new();
...
fn cmd_logs(lines: usize, follow: bool) {
    let log_path = ochi_home().join("tui.log");
...
fn cmd_reset(confirm: bool) {
    let ochi_dir = match dirs::home_dir() {
        Some(_) => ochi_home(),
        None => {
            ui::error("Could not determine home directory");
            std::process::exit(1);
    ...
    if !confirm {
        println!("  This will delete all data in {}", ochi_dir.display());
    ...
    match std::fs::remove_dir_all(&ochi_dir) {
        Ok(()) => ui::success(&format!("Removed {}", ochi_dir.display())),
        Err(e) => {
            ui::error(&format!("Failed to remove {}: {e}", ochi_dir.display()));
            std::process::exit(1);
        }
    }
}
...
mod tests {
    ...
    fn test_doctor_skill_registry_loads_bundled() {
        let skills_dir = std::env::temp_dir().join("ochi-doctor-test-skills");
        let mut skill_reg = ochi_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn test_doctor_extension_registry_loads_bundled() {
        let tmp = std::env::temp_dir().join("ochi-doctor-test-ext");
        let _ = std::fs::create_dir_all(&tmp);
        let mut ext_reg = ochi_extensions::registry::IntegrationRegistry::new(&tmp);
...
"#;
    ...
    fn test_doctor_skill_injection_scan_clean() {
        ...
        let warnings = ochi_skills::verify::SkillVerifier::scan_prompt_content(clean_content);
