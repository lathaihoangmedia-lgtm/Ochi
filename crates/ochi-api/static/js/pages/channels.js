// Ochi Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await OchiAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await OchiAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          OchiToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await OchiAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            OchiToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await OchiAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await OchiAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            OchiToast.success(this.setupModal.display_name + ' activated!');
          } else {
            OchiToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          OchiToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        OchiToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      OchiToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await OchiAPI.delete('/api/channels/' + name + '/configure');
          OchiToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await OchiAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          OchiToast.success(result.message);
        } else {
          OchiToast.error(result.message);
        }
      } catch(e) {
        OchiToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        OchiToast.success('Copied to clipboard');
      } catch(e) {
        OchiToast.error('Copy failed');
      }
    },
