...
function schedulerPage() {
  return {
    ...
    async loadJobs() {
      var data = await OchiAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await OchiAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        OchiToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        OchiToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await OchiAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true };
        OchiToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        OchiToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await OchiAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        OchiToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        OchiToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      OchiToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await OchiAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          OchiToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          OchiToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await OchiAPI.post('/api/schedules/' + job.id + '/run', {});
        if (result.status === 'completed') {
          OchiToast.success('Schedule "' + (job.name || 'job') + '" executed successfully');
          job.last_run = new Date().toISOString();
        } else {
          OchiToast.error('Schedule run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        OchiToast.error('Run Now is not yet available for cron jobs');
      }
      this.runningJobId = '';
    },
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await OchiAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        OchiToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        OchiToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      OchiToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await OchiAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          OchiToast.success('Trigger deleted');
        } catch(e) {
          OchiToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
