// Ochi Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await OchiAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await OchiAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await OchiAPI.get('/api/usage/daily');
