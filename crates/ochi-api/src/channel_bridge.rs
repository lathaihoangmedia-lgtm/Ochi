...
use ochi_channels::discord::DiscordAdapter;
use ochi_channels::email::EmailAdapter;
use ochi_channels::google_chat::GoogleChatAdapter;
use ochi_channels::irc::IrcAdapter;
use ochi_channels::matrix::MatrixAdapter;
use ochi_channels::mattermost::MattermostAdapter;
use ochi_channels::rocketchat::RocketChatAdapter;
use ochi_channels::router::AgentRouter;
use ochi_channels::signal::SignalAdapter;
use ochi_channels::slack::SlackAdapter;
use ochi_channels::teams::TeamsAdapter;
use ochi_channels::telegram::TelegramAdapter;
use ochi_channels::twitch::TwitchAdapter;
use ochi_channels::types::ChannelAdapter;
use ochi_channels::whatsapp::WhatsAppAdapter;
use ochi_channels::xmpp::XmppAdapter;
use ochi_channels::zulip::ZulipAdapter;
// Wave 3
use ochi_channels::bluesky::BlueskyAdapter;
use ochi_channels::feishu::FeishuAdapter;
use ochi_channels::line::LineAdapter;
use ochi_channels::mastodon::MastodonAdapter;
use ochi_channels::messenger::MessengerAdapter;
use ochi_channels::reddit::RedditAdapter;
use ochi_channels::revolt::RevoltAdapter;
use ochi_channels::viber::ViberAdapter;
// Wave 4
use ochi_channels::flock::FlockAdapter;
use ochi_channels::guilded::GuildedAdapter;
use ochi_channels::keybase::KeybaseAdapter;
use ochi_channels::nextcloud::NextcloudAdapter;
use ochi_channels::nostr::NostrAdapter;
use ochi_channels::pumble::PumbleAdapter;
use ochi_channels::threema::ThreemaAdapter;
use ochi_channels::twist::TwistAdapter;
use ochi_channels::webex::WebexAdapter;
// Wave 5
use async_trait::async_trait;
use ochi_channels::dingtalk::DingTalkAdapter;
use ochi_channels::discourse::DiscourseAdapter;
use ochi_channels::gitter::GitterAdapter;
use ochi_channels::gotify::GotifyAdapter;
use ochi_channels::linkedin::LinkedInAdapter;
...
impl ChannelBridgeHandle for KernelBridgeAdapter {
    ...
    async fn spawn_agent_by_name(&self, manifest_name: &str) -> Result<AgentId, String> {
        // Look for manifest at ~/.ochi/agents/{name}/agent.toml
    ...
    async fn list_skills_text(&self) -> String {
        ...
        if skills.is_empty() {
            return "No skills installed. Place skills in ~/.ochi/skills/ or install from the marketplace.".to_string();
        }
    ...
    async fn authorize_channel_user(
        ...
        let auth_action = match action {
            "chat" => ochi_kernel::auth::Action::ChatWithAgent,
            "spawn" => ochi_kernel::auth::Action::SpawnAgent,
            "kill" => ochi_kernel::auth::Action::KillAgent,
            "install_skill" => ochi_kernel::auth::Action::InstallSkill,
            _ => ochi_kernel::auth::Action::ChatWithAgent,
        };
    ...
    async fn record_delivery(
        ...
        let receipt = if success {
            ochi_kernel::DeliveryTracker::sent_receipt(channel, recipient)
        } else {
            ochi_kernel::DeliveryTracker::failed_receipt(
                channel,
                recipient,
...
fn parse_trigger_pattern(s: &str) -> Option<ochi_kernel::triggers::TriggerPattern> {
    use ochi_kernel::triggers::TriggerPattern;
...
pub async fn reload_channels_from_disk(
    ...
    let fresh_config = ochi_kernel::config::load_config(Some(&config_path));
