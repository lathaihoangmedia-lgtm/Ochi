...
use axum::Json;
use dashmap::DashMap;
use ochi_kernel::triggers::{TriggerId, TriggerPattern};
        ...
        match state.kernel.verify_signed_manifest(signed_json) {
            ...
            Err(e) => {
                ...
                state.kernel.audit_log.record(
                    "system",
                    ochi_runtime::audit::AuditAction::AuthAttempt,
...
pub async fn shutdown(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    state.kernel.audit_log.record(
        "system",
        ochi_runtime::audit::AuditAction::ConfigChange,
...
const CHANNEL_REGISTRY: &[ChannelMeta] = &[
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "user_id", label: "Bot User ID", field_type: FieldType::Text, env_var: None, required: false, placeholder: "@ochi:matrix.org", advanced: true },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi_bot", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Bot Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi_bot", advanced: false },
            ...
            ChannelField { key: "subreddits", label: "Subreddits", field_type: FieldType::List, env_var: None, required: false, placeholder: "ochi, rust", advanced: true },
            ChannelField { key: "default_agent", label: "Default Agent", field_type: FieldType::Text, env_var: None, required: false, placeholder: "assistant", advanced: true },
        ],
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi", advanced: false },
            ChannelField { key: "channels", label: "Channels", field_type: FieldType::List, env_var: None, required: false, placeholder: "#ochi, #general", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "nick", label: "Bot Nickname", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi", advanced: false },
    ...
    ChannelMeta {
        ...
        setup_type: "form",
        fields: &[
            ChannelField { key: "topic", label: "Topic", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi-alerts", advanced: false },
    ...
    ChannelMeta {
        ...
        fields: &[
            ...
            ChannelField { key: "username", label: "Username", field_type: FieldType::Text, env_var: None, required: true, placeholder: "ochi", advanced: false },
...
pub async fn configure_channel(
    ...
    let home = ochi_kernel::config::ochi_home();
...
pub async fn whatsapp_qr_start() -> impl IntoResponse {
    ...
    if gateway_url.is_empty() {
        return Json(serde_json::json!({
            ...
            "help": "Run: npx ochi-whatsapp-gateway   (or set WHATSAPP_WEB_GATEWAY_URL)"
        }));
    }
...
pub async fn list_templates() -> impl IntoResponse {
    let agents_dir = ochi_kernel::config::ochi_home().join("agents");
...
/// - `ochi_tool_calls_total` — total tool calls (per agent)
/// - `ochi_panics_total` — supervisor panic count
/// - `ochi_restarts_total` — supervisor restart count
pub async fn prometheus_metrics(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let mut out = String::with_capacity(2048);

    // Uptime
    let uptime = state.started_at.elapsed().as_secs();
    out.push_str("# HELP ochi_uptime_seconds Time since daemon started.\n");
    out.push_str("# TYPE ochi_uptime_seconds gauge\n");
    out.push_str(&format!("ochi_uptime_seconds {uptime}\n\n"));
    ...
    out.push_str(&format!("ochi_agents_active {active}\n"));
    out.push_str("# HELP ochi_agents_total Total number of registered agents.\n");
    out.push_str("# TYPE ochi_agents_total gauge\n");
    out.push_str(&format!("ochi_agents_total {}\n\n", agents.len()));

    // Per-agent token and tool usage
    out.push_str("# HELP ochi_tokens_total Total tokens consumed (rolling hourly window).\n");
    out.push_str("# TYPE ochi_tokens_total gauge\n");
    out.push_str("# HELP ochi_tool_calls_total Total tool calls (rolling hourly window).\n");
    out.push_str("# TYPE ochi_tool_calls_total gauge\n");
    for agent in &agents {
        let name = &agent.name;
        ...
        if let Some((tokens, tools)) = state.kernel.scheduler.get_usage(agent.id) {
            out.push_str(&format!(
                "ochi_tokens_total{{agent=\"{name}\",provider=\"{provider}\",model=\"{model}\"}} {tokens}\n"
            ));
            out.push_str(&format!(
                "ochi_tool_calls_total{{agent=\"{name}\"}} {tools}\n"
            ));
        }
    }
    out.push('\n');
    ...
    // Supervisor health
    let health = state.kernel.supervisor.health();
    out.push_str("# HELP ochi_panics_total Total supervisor panics since start.\n");
    out.push_str("# TYPE ochi_panics_total counter\n");
    out.push_str(&format!("ochi_panics_total {}\n", health.panic_count));
    out.push_str("# HELP ochi_restarts_total Total supervisor restarts since start.\n");
    out.push_str("# TYPE ochi_restarts_total counter\n");
    out.push_str(&format!(
        "ochi_restarts_total {}\n\n",
...
pub async fn list_skills(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let mut registry = ochi_skills::registry::SkillRegistry::new(skills_dir);
    ...
    let skills: Vec<serde_json::Value> = registry
        ...
        .map(|s| {
            let source = match &s.manifest.source {
                Some(ochi_skills::SkillSource::ClawHub { slug, version }) => {
                    serde_json::json!({"type": "clawhub", "slug": slug, "version": version})
                }
                Some(ochi_skills::SkillSource::OpenClaw) => {
                    serde_json::json!({"type": "openclaw"})
                }
                Some(ochi_skills::SkillSource::Bundled) => {
                    serde_json::json!({"type": "bundled"})
                }
                Some(ochi_skills::SkillSource::Native) | None => {
...
pub async fn install_skill(
    ...
    let skills_dir = state.kernel.config.home_dir.join("skills");
    let config = ochi_skills::marketplace::MarketplaceConfig::default();
    let client = ochi_skills::marketplace::MarketplaceClient::new(config);
...
pub async fn clawhub_search(
    ...
    let client = ochi_skills::clawhub::ClawHubClient::new(cache_dir);
...
pub async fn clawhub_browse(
    ...
    let sort = match params.get("sort").map(|s| s.as_str()) {
        Some("downloads") => ochi_skills::clawhub::ClawHubSort::Downloads,
        Some("stars") => ochi_skills::clawhub::ClawHubSort::Stars,
        Some("updated") => ochi_skills::clawhub::ClawHubSort::Updated,
        Some("rating") => ochi_skills::clawhub::ClawHubSort::Rating,
        _ => ochi_skills::clawhub::ClawHubSort::Trending,
    };
...
fn clawhub_browse_entry_to_json(
    entry: &ochi_skills::clawhub::ClawHubBrowseEntry,
) -> serde_json::Value {
    let version = ochi_skills::clawhub::ClawHubClient::entry_version(entry);
    serde_json::json!({
        "slug": entry.slug,
...
pub async fn activate_hand(
    State(state): State<Arc<AppState>>,
    Path(hand_id): Path<String>,
    body: Option<Json<ochi_hands::ActivateHandRequest>>,
) -> impl IntoResponse {
...
pub async fn hand_instance_browser(
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            ochi_runtime::browser::BrowserCommand::ReadPage,
        )
        .await
    {
    ...
    match state
        ...
        .send_command(
            &agent_id_str,
            ochi_runtime::browser::BrowserCommand::Screenshot,
        )
        .await
    {
        Ok(resp) if resp.success => {
...
pub async fn agent_budget_status(
    ...
    let usage_store = ochi_memory::usage::UsageStore::new(state.kernel.memory.usage_conn());
...
pub async fn migrate_detect() -> impl IntoResponse {
    match ochi_migrate::openclaw::detect_openclaw_home() {
        Some(path) => {
            let scan = ochi_migrate::openclaw::scan_openclaw_workspace(&path);
            (
                StatusCode::OK,
                Json(serde_json::json!({
                    "detected": true,
...
pub async fn run_migrate(Json(req): Json<MigrateRequest>) -> impl IntoResponse {
    let source = match req.source.as_str() {
        "openclaw" => ochi_migrate::MigrateSource::OpenClaw,
        "langchain" => ochi_migrate::MigrateSource::LangChain,
        "autogpt" => ochi_migrate::MigrateSource::AutoGpt,
        other => {
            return (
                StatusCode::BAD_REQUEST,
                Json(
    ...
    let options = ochi_migrate::MigrateOptions {
        source,
    ...
    match ochi_migrate::run_migration(&options) {
        Ok(report) => {
...
pub async fn list_providers(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    for p in &provider_list {
        ...
        if !p.key_required {
            ...
            let probe = ochi_runtime::provider_health::probe_provider(&p.id, &p.base_url).await;
...
///
/// Persists to `~/.ochi/custom_models.json` and makes the model immediately
...
pub async fn a2a_agent_card(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    if let Some(first) = agents.first() {
        let card = ochi_runtime::a2a::build_agent_card(&first.manifest, &base_url);
        (
            StatusCode::OK,
...
pub async fn a2a_list_agents(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let cards: Vec<serde_json::Value> = agents
        .iter()
        .map(|entry| {
            let card = ochi_runtime::a2a::build_agent_card(&entry.manifest, &base_url);
...
pub async fn a2a_send_task(
    ...
    let task = ochi_runtime::a2a::A2aTask {
        id: task_id.clone(),
        session_id: session_id.clone(),
        status: ochi_runtime::a2a::A2aTaskStatus::Working,
        messages: vec![ochi_runtime::a2a::A2aMessage {
            role: "user".to_string(),
            parts: vec![ochi_runtime::a2a::A2aPart::Text {
    ...
    match state.kernel.send_message(agent.id, &message_text).await {
        Ok(result) => {
            let response_msg = ochi_runtime::a2a::A2aMessage {
        ...
        Err(e) => {
            let error_msg = ochi_runtime::a2a::A2aMessage {
                role: "agent".to_string(),
                parts: vec![ochi_runtime::a2a::A2aPart::Text {
                    text: format!("Error: {e}"),
                }],
...
pub async fn a2a_discover_external(
    ...
    let client = ochi_runtime::a2a::A2aClient::new();
...
pub async fn mcp_http(
    ...
    if method == "tools/call" {
        ...
        let kernel_handle: Arc<dyn ochi_runtime::kernel_handle::KernelHandle> =
            state.kernel.clone() as Arc<dyn ochi_runtime::kernel_handle::KernelHandle>;
        let result = ochi_runtime::tool_runner::execute_tool(
            "mcp-http",
            tool_name,
            &arguments,
            Some(&kernel_handle),
    ...
    let response = ochi_runtime::mcp_server::handle_mcp_request(&request, &tools).await;
    Json(response)
}
...
pub async fn get_agent_mcp_servers(
    ...
    if let Ok(mcp_tools) = state.kernel.mcp_tools.lock() {
        ...
        for tool in mcp_tools.iter() {
            if let Some(server) = ochi_runtime::mcp::extract_mcp_server(&tool.name) {
...
///
/// SECURITY: Writes to `~/.ochi/secrets.env`, sets env var in process,
...
pub async fn test_provider(
    ...
    let driver_config = ochi_runtime::llm_driver::DriverConfig {
        provider: name.clone(),
        api_key,
    ...
    match ochi_runtime::drivers::create_driver(&driver_config) {
        Ok(driver) => {
...
pub async fn set_provider_url(
    ...
    let probe =
        ochi_runtime::provider_health::probe_provider(&name, &base_url).await;
...
pub async fn create_skill(
    ...
    // Write skill.toml to ~/.ochi/skills/{name}/
    ...
    for info in registry.list_all_info() {
        ...
        let status = match &info.installed {
            ...
            Some(_) => match h.as_ref().map(|h| &h.status) {
                Some(ochi_extensions::IntegrationStatus::Ready) => "ready",
                Some(ochi_extensions::IntegrationStatus::Error(_)) => "error",
                _ => "installed",
            },
...
pub async fn add_integration(
    ...
    let install_err = {
        ...
        if registry.is_installed(&id) {
            ...
            let entry = ochi_extensions::InstalledIntegration {
                id: id.clone(),
...
/// Must match the value in `ochi-kernel/src/kernel.rs::shared_memory_agent_id()`.
...
const SCHEDULES_KEY: &str = "__ochi_schedules";
...
pub async fn serve_upload(Path(file_id): Path<String>) -> impl IntoResponse {
    ...
    let file_path = std::env::temp_dir().join("ochi_uploads").join(&file_id);
...
pub async fn pairing_request(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    match state.kernel.pairing.create_pairing_request() {
        Ok(req) => {
            let qr_uri = format!("ochi://pair?token={}", req.token);
            Json(serde_json::json!({
                "token": req.token,
                "qr_uri": qr_uri,
...
pub async fn pairing_complete(
    ...
    let push_token = body
        ...
        .and_then(|v| v.as_str())
        .map(String::from);
    let device_info = ochi_kernel::pairing::PairedDevice {
...
pub async fn copilot_oauth_start() -> impl IntoResponse {
    ...
    match ochi_runtime::copilot_oauth::start_device_flow().await {
        Ok(resp) => {
...
pub async fn copilot_oauth_poll(
    ...
    match ochi_runtime::copilot_oauth::poll_device_flow(&device_code).await {
        ochi_runtime::copilot_oauth::DeviceFlowStatus::Pending => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "pending"})),
        ),
        ochi_runtime::copilot_oauth::DeviceFlowStatus::Complete { access_token } => {
            // Save to secrets.env
            let secrets_path = state.kernel.config.home_dir.join("secrets.env");
        ...
        ochi_runtime::copilot_oauth::DeviceFlowStatus::SlowDown { new_interval } => {
            // Update interval
        ...
        ochi_runtime::copilot_oauth::DeviceFlowStatus::Expired => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
                Json(serde_json::json!({"status": "expired"})),
        ...
        ochi_runtime::copilot_oauth::DeviceFlowStatus::AccessDenied => {
            COPILOT_FLOWS.remove(&poll_id);
            (
                StatusCode::OK,
        ...
        ochi_runtime::copilot_oauth::DeviceFlowStatus::Error(e) => (
            StatusCode::OK,
            Json(serde_json::json!({"status": "error", "error": e})),
        ),
    }
