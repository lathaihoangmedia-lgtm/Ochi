        ...
        if let Some(ref entries) = skills.entries {
            if !entries.is_empty() {
                report.skipped.push(SkippedItem {
                    ...
                    reason: "Skills must be reinstalled via `ochi skill install`".to_string(),
                });
            }
        }
    }
            ...
            for entry in entries.flatten() {
                ...
                if has_package_json && has_index {
                    report.skipped.push(SkippedItem {
                        kind: ItemKind::Skill,
                        name: name.clone(),
                        reason: "Node.js skill — run with `ochi skill install` after migration"
                            .to_string(),
                    });
                } else {
                    report.skipped.push(SkippedItem {
...
mod tests {
  ...
  agents: {
    ...
    fn test_is_known_ochi_tool() {
        assert!(is_known_ochi_tool("file_read"));
        assert!(is_known_ochi_tool("shell_exec"));
        assert!(is_known_ochi_tool("web_fetch"));
