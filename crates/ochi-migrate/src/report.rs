...
impl MigrationReport {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        out.push_str("1. Review imported agent manifests in `~/.ochi/agents/`\n");
        out.push_str(
            "2. Review `~/.ochi/secrets.env` — verify tokens were migrated correctly\n",
        );
        out.push_str("3. Set any remaining API keys referenced in `~/.ochi/config.toml`\n");
        out.push_str("4. Start the daemon: `ochi start`\n");
        out.push_str("5. Test your agents: `ochi agent list`\n");
    ...
    fn test_report_with_items() {
        let report = MigrationReport {
            ...
            imported: vec![MigrateItem {
                ...
                destination: "~/.ochi/agents/coder/agent.toml".to_string(),
            }],
