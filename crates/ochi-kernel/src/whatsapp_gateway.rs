...
fn gateway_dir() -> PathBuf {
    ochi_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::OchiKernel>) {
    ...
    let ochi_url = format!("http://{api_listen}");
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@ochi/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_ochi_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("ochi_test_gateway");
