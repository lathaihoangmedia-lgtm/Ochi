...
use ochi_runtime::agent_loop::{
    run_agent_loop, run_agent_loop_streaming, strip_provider_prefix, AgentLoopResult,
};
use ochi_runtime::audit::AuditLog;
use ochi_runtime::drivers;
use ochi_runtime::kernel_handle::{self, KernelHandle};
use ochi_runtime::llm_driver::{CompletionRequest, DriverConfig, LlmDriver, StreamEvent};
use ochi_runtime::python_runtime::{self, PythonConfig};
...
pub struct OpenFangKernel {
    ...
    pub model_catalog: std::sync::RwLock<ochi_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<ochi_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<ochi_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: ochi_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, ochi_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: ochi_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: ochi_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: ochi_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: ochi_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn ochi_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Hand registry — curated autonomous capability packages.
    pub hand_registry: ochi_hands::registry::HandRegistry,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<ochi_extensions::registry::IntegrationRegistry>,
    ...
    pub hooks: ochi_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<ochi_runtime::process_manager::ProcessManager>,
    /// OFP peer registry — tracks connected peers.
    pub peer_registry: Option<ochi_wire::PeerRegistry>,
    /// OFP peer node — the local networking node.
    pub peer_node: Option<Arc<ochi_wire::PeerNode>>,
...
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<ochi_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: ochi_channels::types::DeliveryReceipt) {
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<ochi_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> ochi_channels::types::DeliveryReceipt {
        ochi_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: ochi_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        ochi_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: ochi_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
impl OpenFangKernel {
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("ochi.db"));
        let memory = Arc::new(
        ...
        let driver: Arc<dyn LlmDriver> = if !config.fallback_providers.is_empty() {
            ...
            if chain.len() > 1 {
                Arc::new(ochi_runtime::drivers::fallback::FallbackDriver::new(
                    chain,
                ))
            } else {
                primary_driver
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            ochi_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = ochi_runtime::model_catalog::ModelCatalog::new();
        ...
        // Load user's custom models from ~/.ochi/custom_models.json
        ...
        let mut skill_registry = ochi_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let mut hand_registry = ochi_hands::registry::HandRegistry::new();
        ...
        let mut extension_registry =
            ochi_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = ochi_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = ochi_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(ochi_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = ochi_runtime::web_search::WebToolsContext {
            search: ochi_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
            fetch: ochi_runtime::web_fetch::WebFetchEngine::new(
        ...
        let embedding_driver: Option<
            Arc<dyn ochi_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use ochi_runtime::embedding::create_embedding_driver;
        ...
        let browser_ctx = ochi_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            ochi_runtime::media_understanding::MediaEngine::new(config.media.clone());
        let tts_engine = ochi_runtime::tts::TtsEngine::new(config.tts.clone());
        ...
        let kernel = Self {
            ...
            a2a_task_store: ochi_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: ochi_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(ochi_runtime::process_manager::ProcessManager::new(5)),
            peer_registry: None,
            peer_node: None,
    ...
    pub fn spawn_agent_with_parent(
        ...
        // This ensures agents respect the user's configured provider from `ochi init`.
        ...
        self.audit_log.record(
            agent_id.to_string(),
            ochi_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub async fn send_message_with_handle(
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    ochi_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            let prompt_ctx = ochi_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        ochi_runtime::workspace_context::WorkspaceContext::detect(w);
            ...
            manifest.model.system_prompt =
                ochi_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
            ...
            if let Some(cc_msg) =
                ochi_runtime::prompt_builder::build_canonical_context_message(&prompt_ctx)
            {
        ...
        let message_owned = if let Some(link_ctx) =
            ochi_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: ochi_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use ochi_runtime::agent_loop::LoopPhase;
    ...
    async fn execute_llm_agent(
        ...
        let _ = self.metering.record(&ochi_memory::usage::UsageRecord {
            agent_id,
            model: model.clone(),
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = ochi_runtime::mcp::extract_mcp_server(&tool.name) {
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        ...
        let (repaired_messages, repair_stats) =
            ochi_runtime::session_repair::validate_and_repair_with_stats(&result.kept_messages);
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<ochi_runtime::compactor::ContextReport> {
        use ochi_runtime::compactor::generate_context_report;
        use ochi_runtime::tool_runner::builtin_tool_definitions;
    ...
    pub fn kill_agent(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        self.audit_log.record(
            agent_id.to_string(),
            ochi_runtime::audit::AuditAction::AgentKill,
    ...
    pub fn activate_hand(
    ...
    ) -> KernelResult<ochi_hands::HandInstance> {
        use ochi_hands::HandError;
        ...
        let resolved = ochi_hands::resolve_settings(&def.settings, &instance.config);
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        // Probe local providers for reachability and model discovery
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        ochi_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered = ochi_runtime::a2a::discover_external_agents(&agents).await;
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use ochi_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "ochi-node".to_string());
        ...
        let handle: Arc<dyn ochi_wire::peer::PeerHandle> = self.self_arc();
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        if !manifest.fallback_models.is_empty() {
            ...
            if chain.len() > 1 {
                return Ok(Arc::new(
                    ochi_runtime::drivers::fallback::FallbackDriver::new(chain),
                ));
            }
        }
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| ochi_runtime::mcp::normalize_name(s))
                    .collect();
                all_tools.extend(
                    mcp_tools
                        .iter()
                        .filter(|t| {
                            ochi_runtime::mcp::extract_mcp_server(&t.name)
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = ochi_skills::registry::SkillRegistry::new(skills_dir);
...
impl KernelHandle for OpenFangKernel {
    ...
    async fn send_channel_message(
        ...
        let user = ochi_channels::types::ChannelUser {
        ...
        adapter
            .send(&user, ochi_channels::types::ChannelContent::Text(message.to_string()))
            .await
...
impl ochi_wire::peer::PeerHandle for OpenFangKernel {
    fn local_agents(&self) -> Vec<ochi_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| ochi_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<ochi_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
