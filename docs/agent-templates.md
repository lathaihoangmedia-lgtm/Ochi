...
Ochi ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
ochi spawn orchestrator
ochi spawn coder
ochi spawn --template agents/writer/agent.toml
```
...
> General-purpose assistant. The default Ochi agent for everyday tasks, questions, and conversations.
...
ochi spawn coder --name "backend-coder"

# Spawn from a TOML file path
ochi spawn --template agents/custom/my-agent.toml

# List running agents
ochi agents

# Send a message
ochi message <agent-id> "Write a function to parse TOML files"
