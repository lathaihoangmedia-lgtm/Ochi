...
- Chạy kiểm chứng kỹ thuật:
  - `cargo test -p ochi-types` (pass).
  - `cargo check -p ochi-cli` (pass, có warnings và future-incompat notice).
...
- Có test unit dày ở tầng kiểu dữ liệu/chính sách (`ochi-types`: 273 test pass).
- Tài liệu vận hành và release được viết có cấu trúc, có checklist rõ ràng.

**Hạn chế**

- `ochi-cli` hiện còn nhiều `unused variable` warnings.
...
3. Chốt domain phân phối install script (`ochi.sh`) và xác minh đường dẫn.
...
1. **Chốt và kiểm chứng install endpoint (`ochi.sh`)**
   - Việc làm:
...
3. **Dọn warning dễ sửa ở `ochi-cli`**
   - Việc làm:
     - Xử lý nhóm `unused variable` và import thừa.
     - Chạy lại `cargo check -p ochi-cli` để xác nhận giảm nhiễu cảnh báo.
...
- `cargo check -p ochi-cli` chạy lại với số warning giảm rõ rệt.
...
4. P1 — Chuẩn bị batch tiếp theo: Python package rename (`ochi` -> `ochi`) kèm metapackage/alias.
