# Getting Started with Ochi

This guide walks you through installing Ochi, configuring your first LLM provider, spawning an agent, and chatting with it.
...
Download the installer for your platform from the [latest release](https://github.com/lathaihoangmedia-lgtm/Ochi/releases/latest):
...
The desktop app includes the full Ochi system with a native window, system tray, auto-updates, and OS notifications. Updates are installed automatically in the background.
...
This downloads the latest CLI binary and installs it to `~/.ochi/bin/`.
...
```bash
cargo install --git https://github.com/lathaihoangmedia-lgtm/Ochi ochi-cli
```
...
```bash
docker pull ghcr.io/RightNow-AI/ochi:latest
...
docker run -d \
  ...
  -v ochi-data:/data \
  ghcr.io/RightNow-AI/ochi:latest
```
...
Run the init command to create the `~/.ochi/` directory and a default config file:
...
Ochi needs at least one LLM provider API key. Set it as an environment variable:
...
The default config uses Anthropic. To change the provider, edit `~/.ochi/config.toml`:
...
Ochi ships with 30 agent templates. Spawn the hello-world agent:

```bash
ochi agent spawn agents/hello-world/agent.toml
```
...
```bash
ochi agent spawn my-agent.toml
```
...
agent> I'm the hello-world agent running on Ochi. I can:
...
```
Starting Ochi daemon...
Ochi daemon running on http://127.0.0.1:4200
...
Now that you have Ochi running:
...
- **Set up workflows**: Chain multiple agents together. Use `ochi workflow create` with a TOML workflow definition.
- **Use MCP**: Connect to external tools via Model Context Protocol. Configure in `config.toml` under `[[mcp_servers]]`.
- **Migrate from OpenClaw**: Run `ochi migrate --from openclaw`. See [MIGRATION.md](../MIGRATION.md).
- **Desktop app**: Run `cargo tauri dev` for a native desktop experience with system tray.
- **Run diagnostics**: `ochi doctor` checks your entire setup.

### Useful Commands Reference

```bash
ochi init                          # Initialize ~/.ochi/
ochi start                         # Start the daemon
ochi status                        # Check daemon status
ochi doctor                        # Run diagnostic checks

ochi agent spawn <manifest.toml>   # Spawn an agent
ochi agent list                    # List all agents
ochi agent chat <id>               # Chat with an agent
ochi agent kill <id>               # Kill an agent

ochi workflow list                 # List workflows
ochi workflow create <file.json>   # Create a workflow
ochi workflow run <id> <input>     # Run a workflow

ochi trigger list                  # List event triggers
ochi trigger create <args>         # Create a trigger
ochi trigger delete <id>           # Delete a trigger

ochi skill install <source>        # Install a skill
ochi skill list                    # List installed skills
ochi skill search <query>          # Search FangHub
ochi skill create                  # Scaffold a new skill

ochi channel list                  # List channel status
ochi channel setup <channel>       # Interactive setup wizard

ochi config show                   # Show current config
ochi config edit                   # Open config in editor

ochi chat [agent]                  # Quick chat (alias)
ochi migrate --from openclaw       # Migrate from OpenClaw
ochi mcp                           # Start MCP server (stdio)
```
