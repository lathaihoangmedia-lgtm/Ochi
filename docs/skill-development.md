...
Skills are pluggable tool bundles that extend agent capabilities in Ochi. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.ochi/skills/` and made available to agents through the skill registry. Ochi ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
Ochi includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. Ochi sends a JSON payload to the script's stdin:
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Ochi-Skill/1.0"})
...
### Using the Ochi Python SDK

For more advanced skills, use the Python SDK (`sdk/python/ochi_sdk.py`; legacy alias: `openfang_sdk.py` (deprecated)):

```python
#!/usr/bin/env python3
from ochi_sdk import SkillHandler
...
FangHub is the community skill marketplace for Ochi.
...
Ochi can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.
...
If the directory contains an OpenClaw-style skill (Node.js package), Ochi:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to Ochi conventions.
4. Copies the skill to the Ochi skills directory.
