...
### `ochi` command not found after install
...
**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/RightNow-AI/ochi`
...
**Fix**: Run `ochi init` to create the default config:
```bash
ochi init
```

This creates `~/.ochi/config.toml` with sensible defaults.
...
**Check**:
1. TOML manifest is valid: `ochi agent spawn --dry-run manifest.toml`
...
2. Set the model to `ochi:agent-name` (e.g., `ochi:coder`)
...
3. Try deleting `~/.ochi/daemon.json` and restarting
...
If slower:
- Check database size (`~/.ochi/data/ochi.db`)
...
1. Add the channel config to `~/.ochi/config.toml` under `[channels]`
...
```bash
# From source
cd ochi && git pull && cargo install --path crates/ochi-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.ochi/data/ochi.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.ochi/config.toml` (configuration)
- `~/.ochi/data/ochi.db` (all agent data, memory, sessions)
...
```toml
[dependencies]
ochi-runtime = { path = "crates/ochi-runtime" }
ochi-memory = { path = "crates/ochi-memory" }
```

The `ochi-kernel` crate assembles everything, but you can use individual crates for custom integrations.
