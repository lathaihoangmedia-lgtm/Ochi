...
Ochi is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
ochi-cli            CLI interface, daemon auto-detect, MCP server
    |
ochi-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
ochi-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
ochi-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- ochi-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- ochi-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- ochi-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- ochi-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- ochi-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
ochi-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
ochi-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `OchiKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.ochi/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.ochi/data/ exists

3. Initialize memory substrate
   - Open SQLite database (ochi.db)
...
The memory substrate (`ochi-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`ochi-runtime`) provides a unified interface for all LLM providers:
...
The `ModelCatalog` (`ochi-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
Ochi implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `ochi-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`ochi-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`ochi-skills`) provides 60 bundled skills and supports external skill installation.
...
- **FangHub**: Native Ochi marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-Ochi tool name mappings in `tool_compat.rs`.
...
Ochi implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Ochi's 23 built-in tools via the MCP protocol. Enables external tools to use Ochi as a tool provider.
...
The Ochi Protocol (OFP) (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`ochi-desktop`) wraps the full Ochi stack in a native Tauri 2.0 application.
