...
ARG OCHI_SMOKE_FULL=0
RUN if [ "$OCHI_SMOKE_FULL" = "1" ]; then \
        bash /tmp/install.sh; \
    else \
        # 1. Syntax check
...
RUN if [ "$OCHI_SMOKE_FULL" = "1" ] && [ -f "$HOME/.ochi/bin/ochi" ]; then \
        $HOME/.ochi/bin/ochi --version && \
        echo "PASS: ochi binary works"; \
    else \
