...
async function startConnection() {
  ...
  // Incoming messages → forward to Ochi
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to Ochi agent
      try {
        const response = await forwardToOchi(text, phone, pushName);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to Ochi API, return agent response
// ---------------------------------------------------------------------------
function forwardToOchi(text, phone, pushName) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${OCHI_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Ochi API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by Ochi for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] Ochi URL: ${OCHI_URL}`);
