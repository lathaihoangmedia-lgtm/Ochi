...
```bash
GROQ_API_KEY=<key> target/release/ochi.exe start &
...
## Architecture Notes
- **Don't touch `ochi-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `ochi.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
